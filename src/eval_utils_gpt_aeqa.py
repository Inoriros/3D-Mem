...
def call_ollama_api(sys_prompt, contents, model="qwen2.5-vl:7b", ollama_url="http://localhost:11434") -> Optional[str]:
    ...
    Returns:
        Optional[str]: Model response content or None if failed
    """
    max_tries = 5
    retry_count = 0
    
    # Convert contents to Ollama format
    def format_content_for_ollama(contents):
    ...
    text_content, images = format_content_for_ollama(contents)
    ...
    while retry_count < max_tries:
        try:
            ...
            print("Ollama API timeout, retrying...")
            retry_count += 1
            time.sleep(10)
            continue
        except requests.exceptions.ConnectionError:
            print("Ollama connection error, waiting for 30s")
            time.sleep(30)
            retry_count += 1
            continue
        except Exception as e:
...
# Alternative function that uses the same message format as OpenAI for easier switching
def call_ollama_api_openai_format(sys_prompt, contents, model="qwen2.5vl:7b", ollama_url="http://localhost:11434") -> Optional[str]:
    """
    Call Ollama API using OpenAI-like message format (for future compatibility)
    ...
    # Convert OpenAI format to Ollama chat format
    messages = [
        {"role": "system", "content": sys_prompt},
        {"role": "user", "content": formated_content},
    ]
...
# Drop-in replacement for call_openai_api using Ollama
def call_openai_api_ollama_replacement(sys_prompt, contents) -> Optional[str]:
    """
    Drop-in replacement for call_openai_api that uses Ollama qwen2.5-vl:7b instead
    Maintains the exact same function signature and behavior
    """
    return call_ollama_api(sys_prompt, contents)


# Smart wrapper that chooses between OpenAI and Ollama based on configuration
def call_api_smart(sys_prompt, contents) -> Optional[str]:
    """
    Smart API caller that chooses between OpenAI and Ollama based on USE_OLLAMA setting
    """
    from src.const import USE_OLLAMA, OLLAMA_URL, OLLAMA_MODEL
    
    if USE_OLLAMA:
        return call_ollama_api(sys_prompt, contents, model=OLLAMA_MODEL, ollama_url=OLLAMA_URL)
    else:
        return call_openai_api(sys_prompt, contents)
